...
# COLLECTION_ARCHIVE@
curl -k -X POST --user crosslang:isthebest -H 'Content-type:application/json' --data-binary "$JSON" $SOLR_HOST/solr/$COLLECTION_ARCHIVE/schema
curl -k -X POST --user crosslang:isthebest -H 'Content-type:application/json' --data-binary "$JSON_ARCHIVE" $SOLR_HOST/solr/$COLLECTION_ARCHIVE/schema
curl -k --user crosslang:isthebest $SOLR_HOST/solr/$COLLECTION_ARCHIVE/config -d '{"set-user-property": {"update.autoCreateFields":"false"}}'
