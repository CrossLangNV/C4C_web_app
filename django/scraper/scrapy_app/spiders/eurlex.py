...
class EurLexSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for h2 in response.css("div.EurlexContent div.SearchResult"):
            doc_link = h2.css("::attr(name)").extract_first().replace(
                '/AUTO/', '/EN/ALL/')
    ...
    def parse_document(self, response):
        ...
        if body:
            ...
            if status:
                status = ''.join(status.xpath('.//text()').getall()
                                 ).replace('\n', ' ').strip().split(':')[0]
        ...
        summary_link = response.xpath(
            "//li[@class='legissumTab']/a/@href").get()
        if summary_link:
    ...
    def parse_dates(self, response, result_dict):
        ...
        if dates:
            ...
            for (t, d) in zip(date_types, date_texts):
                ...
                date_value_all = ''.join(
                    d.xpath('.//text()').getall()).split(';')
                if '/' in date_value_all[0]:
                    date_value = datetime.strptime(
                        date_value_all[0], self.date_format)
                else:
    ...
    def parse_classifications(self, response, result_dict):
        ...
        if classifications:
            ...
            for (t, d) in zip(classifications_types, classifications_data):
                ...
                for ref_code in ref_codes:
                    all_classifications_type.append(
                        t.xpath('.//text()').get().split(':')[0].lower())
                    element_name = ''.join(ref_code.xpath(
                        './/text()').getall()).replace('\n', ' ').strip()
                    ref_code = str(ref_code.xpath('.//@href').get()
                                   ).replace('./../../../', base_url)
            ...
            result_dict.update(
                {"classifications_type": all_classifications_type})
            result_dict.update(
                {"classifications_code": all_classifications_code})
    ...
    def parse_procedures(self, response, result_dict):
        ...
        if procedures:
            ...
            result_dict.update(
                {"procedures_links_url": all_procedures_links_url})
    ...
    def parse_relationships(self, response, result_dict):
        ...
        if relationships:
            ...
            for rel_text in rel_texts[1:]:
                ...
                rel_category = rel_types[common_index].xpath(
                    './/text()').get().split(':')[0].replace('\n', '').lower()
                redirect_link_value = ''
                redirect_links = rel_text.xpath('.//a')
                for redirect_link in redirect_links:
                    redirect_link_text = redirect_link.xpath('.//text()').get()
                    redirect_link_url = redirect_link.xpath(
                        './@href').get().replace('./../../../', base_url)
            ...
            if amendments:
                ...
                if rows:
                    ...
                    for tr in rows:
                        td = tr.xpath('.//td')
                        relation = td[0].xpath(
                            './/text()').get(default='').replace('\n', ' ').strip()
                        act = td[1].xpath(
                            './/a//text()').get(default='').replace('\n', ' ').strip()
                        comment = td[2].xpath(
                            './/text()').get(default='').replace('\n', ' ').strip()
                        subdivision_concerned = td[3].xpath(
                            './/text()').get(default='').replace('\n', ' ').strip()
                        as_from = td[4].xpath(
                            './/text()').get(default='').replace('\n', ' ').strip()
                        to = td[5].xpath(
                    ...
                    result_dict.update(
                        {'amendments_from': all_amendments_from})
